    ...
    public static class VFavorites
    {
        ...
        static void SelectItem(Item item)
        {
            void openFolder(EditorWindow browser, string path)
            {
                ...
                var folderInstanceId = folderAsset.GetInstanceID();
                ...
                else
                {
                    ...
                    TryInvoke(browser.GetMemberValue("m_AssetTree")?.GetPropertyValue("data", exceptionIfNotFound: false), "SetExpanded", folderInstanceId, true);
        ...
        static bool TryInvoke(object target, string methodName, params object[] parameters)
        {
            ...
            foreach (var methodInfo in GetMethods(type, methodName, parameters.Length))
            {
                if (!TryConvertParameters(methodInfo, parameters, out var convertedParameters)) continue;

                try
                {
                    methodInfo.Invoke(target is Type ? null : target, convertedParameters);
                    return true;
                }
                catch { }
            }
        ...
        static bool TrySetSelectedInstanceId(object listAreaState, int folderInstanceId)
        {
            if (listAreaState == null) return false;

            var fieldInfo = listAreaState.GetType().GetField("m_SelectedInstanceIDs", maxBindingFlags);

            if (fieldInfo == null) return false;

            var listType = fieldInfo.FieldType;
            var itemType = listType.IsGenericType ? listType.GetGenericArguments()[0] : typeof(int);

            if (!TryConvertValue(folderInstanceId, itemType, out var convertedInstanceId)) return false;

            if (!(System.Activator.CreateInstance(typeof(List<>).MakeGenericType(itemType)) is IList selectedIds)) return false;

            selectedIds.Add(convertedInstanceId);

            try
            {
                fieldInfo.SetValue(listAreaState, selectedIds);
                return true;
            }
            catch { return false; }
        ...
        static bool TryConvertParameters(MethodInfo methodInfo, object[] parameters, out object[] convertedParameters)
        {
            ...
            for (var i = 0; i < parameters.Length; i++)
                if (TryConvertValue(parameters[i], parameterInfos[i].ParameterType, out var convertedParameter))
        ...
        static bool TryConvertValue(object value, Type targetType, out object convertedValue)
        {
            convertedValue = null;
            ...
            if (targetType.IsArray && value is IEnumerable enumerable)
            {
                ...
                {
                    if (!TryConvertValue(item, itemType, out var convertedItem)) return false;

                    convertedItems.Add(convertedItem);
                }

                var array = System.Array.CreateInstance(itemType, convertedItems.Count);

                for (var i = 0; i < convertedItems.Count; i++)
            ...
            if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(List<>) && value is IEnumerable listEnumerable)
            {
                var itemType = targetType.GetGenericArguments()[0];

                if (!(System.Activator.CreateInstance(targetType) is IList list)) return false;
            ...
            if (value is int instanceId && TryConvertInstanceId(instanceId, targetType, out convertedValue))
                return true;
        ...
        static bool TryConvertInstanceId(int instanceId, Type targetType, out object convertedValue)
        {
            convertedValue = null;
            ...
            foreach (var methodInfo in targetType.GetMethods(maxBindingFlags).Where(r => r.IsStatic && (r.Name == "op_Implicit" || r.Name == "op_Explicit")))
            {
                var parameters = methodInfo.GetParameters();

                if (parameters.Length != 1) continue;
                if (parameters[0].ParameterType != typeof(int)) continue;
                if (methodInfo.ReturnType != targetType) continue;

                try
                {
                    convertedValue = methodInfo.Invoke(null, new object[] { instanceId });
                    return true;
                }
                catch { }
            }

            foreach (var constructorInfo in targetType.GetConstructors(maxBindingFlags))
            {
                ...
                try
                {
                    convertedValue = constructorInfo.Invoke(new object[] { instanceId });
                    return true;
                }
                catch { }
            }
            ...
            try
            {
                var value = System.Activator.CreateInstance(targetType);
                var intField = targetType.GetFields(maxBindingFlags).FirstOrDefault(r => r.FieldType == typeof(int) || r.FieldType == typeof(long));

                if (intField == null) return false;

                intField.SetValue(value, intField.FieldType == typeof(long) ? (object)(long)instanceId : instanceId);
                convertedValue = value;
                return true;
            }
            catch { return false; }
        ...
        static IEnumerable<MethodInfo> GetMethods(Type type, string methodName, int argumentCount)
        {
            IEnumerable<MethodInfo> getMethods(Type curType)
            {
                foreach (var methodInfo in curType.GetMethods(maxBindingFlags))
                {
            ...
            for (var curType = type; curType != null; curType = curType.BaseType)
                foreach (var methodInfo in getMethods(curType))
                    yield return methodInfo;

            foreach (var interfaceType in type.GetInterfaces())
                foreach (var methodInfo in getMethods(interfaceType))
                    yield return methodInfo;
