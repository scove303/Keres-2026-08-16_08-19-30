...
namespace VHierarchy.Libs
{
    public static class VUtils
    {
        ...
        {
            if (sizeUnknownIfNotMoreThanZero && bytes == 0)
                return "Size unknown";

            var ss = new[] { "B", "KB", "MB", "GB", "TB" };
            var bprev = bytes;
            int i = 0;
            while (bytes >= 1024 && i++ < ss.Length - 1)
                bytes = (bprev = bytes) / 1024;
        ...
        public static string FormatTime(long ms, bool includeMs = false)
        {
            ...
            var s = "";
            if (t.Hours != 0)
                s += " " + t.Hours + " hour" + CountSuffix(t.Hours);
            if (t.Minutes != 0)
                s += " " + t.Minutes + " minute" + CountSuffix(t.Minutes);
            if (t.Seconds != 0)
                s += " " + t.Seconds + " second" + CountSuffix(t.Seconds);
            if (t.Milliseconds != 0 && includeMs)
                s += " " + t.Milliseconds + " millisecond" + CountSuffix(t.Milliseconds);
        ...
        public static TValue GetValueOrDefault<TKey, TValue>(
            ...
            TKey key
        ) => dic.ContainsKey(key) ? dic[key] : default;
        ...
        )
        {
            ...
            if (exceptionIfNotFound)
                throw new System.Exception(
                    $"Field '{fieldName}' not found in '{type.Name}' type and its parent types"
                );
        ...
        )
        {
            ...
            if (exceptionIfNotFound)
                throw new System.Exception(
                    $"Property '{propertyName}' not found in '{type.Name}' type and its parent types"
                );
        ...
        )
        {
            ...
            if (exceptionIfNotFound)
                throw new System.Exception(
                    $"Member '{memberName}' not found in '{type.Name}' type and its parent types"
                );
        ...
        )
        {
            ...
            if (
                type.GetMethodInfo(methodName, parameters.Select(r => r.GetType()).ToArray())
                is MethodInfo methodInfo
            )
                return methodInfo.Invoke(target, parameters);

            throw new System.Exception(
                $"Method '{methodName}' not found in '{type.Name}' type, its parent types and interfaces"
            );
        }
        ...
        )
        {
            ...
            for (var curType = type; curType != null; curType = curType.BaseType)
                if (
                    curType.GetMethod(methodName, maxBindingFlags, null, argumentTypes, null)
                    is MethodInfo methodInfo
                )
            ...
            foreach (var interfaceType in type.GetInterfaces())
                if (
                    interfaceType.GetMethod(methodName, maxBindingFlags, null, argumentTypes, null)
                    is MethodInfo methodInfo
                )
                    return methodInfoCache[type][methodHash] = methodInfo;
        ...
        )
        {
            ...
            if (
                ...
                && val is System.Collections.IEnumerable ienum
                && !(val is List<int>)
            )
            {
        ...
        public static object GetDefaultValue(this FieldInfo f, params object[] constructorVars) =>
            f.GetValue(
                System.Activator.CreateInstance(((MemberInfo)f).ReflectedType, constructorVars)
            );
        ...
        public static IEnumerable<PropertyInfo> GetPropertiesWithoutBase(this Type t) =>
            t.GetProperties().Where(r => !t.BaseType.GetProperties().Any(rr => rr.Name == r.Name));
        ...
        public static Vector3 Clamp01(this Vector3 f) =>
            new Vector3(f.x.Clamp01(), f.y.Clamp01(), f.z.Clamp01());
        ...
        public static float ProjectOn(this Vector2 v, Vector2 on) =>
            Vector3.Project(v, on).magnitude;
        ...
        public static Vector2 Rotate(this Vector2 v, float deg) =>
            Quaternion.AngleAxis(deg, Vector3.forward) * v;
        ...
        public static float Lerp(float current, float target, float speed, float deltaTime) =>
            Mathf.Lerp(current, target, LerpT(speed, deltaTime));
        ...
        public static Vector2 Lerp(Vector2 current, Vector2 target, float speed, float deltaTime) =>
            Vector2.Lerp(current, target, LerpT(speed, deltaTime));
        ...
        public static Vector3 Lerp(Vector3 current, Vector3 target, float speed, float deltaTime) =>
            Vector3.Lerp(current, target, LerpT(speed, deltaTime));
        ...
        public static Color LCHtoRGB(float l, float c, float h)
        {
            ...
            float x = (float)
                System.Math.Round(
                    xw
                        * (
                            (System.Math.Pow(fx, 3) > 0.008856)
                                ? System.Math.Pow(fx, 3)
                                : ((fx - 16 / 116) / 7.787)
                        ),
                    5
                );
            float y = (float)
                System.Math.Round(
                    yw
                        * (
                            (System.Math.Pow(fy, 3) > 0.008856)
                                ? System.Math.Pow(fy, 3)
                                : ((fy - 16 / 116) / 7.787)
                        ),
                    5
                );
            float z = (float)
                System.Math.Round(
                    zw
                        * (
                            (System.Math.Pow(fz, 3) > 0.008856)
                                ? System.Math.Pow(fz, 3)
                                : ((fz - 16 / 116) / 7.787)
                        ),
                    5
                );
        ...
        public static Rect SetMidPos(this Rect r, Vector2 v) =>
            r.SetPos(v).MoveX(-r.width / 2).MoveY(-r.height / 2);
        ...
        public static Rect AddWidthFromMid(this Rect rect, float f) =>
            rect.SetWidthFromMid(rect.width + f);

        public static Rect AddWidthFromRight(this Rect rect, float f) =>
            rect.SetWidthFromRight(rect.width + f);
        ...
        public static Rect AddHeightFromMid(this Rect rect, float f) =>
            rect.SetHeightFromMid(rect.height + f);

        public static Rect AddHeightFromBottom(this Rect rect, float f) =>
            rect.SetHeightFromBottom(rect.height + f);
        ...
        public static RenderTexture CreateRT(
            int width,
            int height,
            GraphicsFormat graphicsFormat = GraphicsFormat.R8G8B8A8_SRGB,
            bool useMips = false,
        ...
        public static RenderTexture CreateRT(
            this RenderTextureDescriptor descriptor,
            int resolution
        )
        {
        ...
        public static RenderTexture CreateRT(
            this RenderTextureDescriptor descriptor,
            float resolution
        ) => descriptor.GetTemporaryRT(Mathf.RoundToInt(resolution));

        public static RenderTexture CreateRT(
            this RenderTextureDescriptor descriptor,
            float width,
            float height
        ) => descriptor.CreateRT(Mathf.RoundToInt(width), Mathf.RoundToInt(height));
        ...
        public static RenderTexture GetTemporaryRT(
            ...
            float width,
            float height
        ) => descriptor.GetTemporaryRT(Mathf.RoundToInt(width), Mathf.RoundToInt(height));
        ...
        {
            var rt = GetTemporaryRT(
                w,
                h,
                texture2d.graphicsFormat.GetCompatibleForRendering(),
                false
            );

            Graphics.Blit(texture2d, rt);

            var resizedCopy = CreateTexture2D(
                w,
                h,
                texture2d.graphicsFormat.GetCompatibleForRendering(),
                texture2d.mipmapCount > 1
        ...
        )
        {
            ...
            if (useCompression != null)
                importer.textureCompression = useCompression.GetValueOrDefault()
        ...
        public static TextureImporter GetImporter(this Texture2D t) =>
            (TextureImporter)AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(t));
        ...
        public static Bounds GetBounds(this GameObject go, bool local = false)
        {
            ...
            {
                var b = local
                    ? r.gameObject.GetComponent<MeshFilter>().sharedMesh.bounds
                    : r.bounds;
            ...
            foreach (var r in go.GetComponentsInChildren<Terrain>())
            {
                ...
                if (bounds == default)
                    ...
                    bounds.Encapsulate(
                        new Bounds(
                            r.transform.position + r.terrainData.size / 2,
                            r.terrainData.size
                        )
                    );
            }
        ...
        public struct GlobalID : System.IEquatable<GlobalID>
        {
            ...
            public GlobalID(Object o) =>
                globalObjectIdString = (
                    _globalObjectId = GlobalObjectId.GetGlobalObjectIdSlow(o)
                ).ToString();
            ...
            public bool Equals(GlobalID other) =>
                this.globalObjectIdString.Equals(other.globalObjectIdString);
            ...
            public override bool Equals(object other) =>
                other is GlobalID otherglobalID && this.Equals(otherglobalID);

            public override int GetHashCode() =>
                globalObjectIdString == null ? 0 : globalObjectIdString.GetHashCode();
            ...
            public GlobalID UnpackForPrefab()
            {
                ...
                var unpackedGId = new GlobalID(
                    $"GlobalObjectId_V1-{this.idType}-{this.guid}-{unpackedFileId}-0"
                );
        ...
        public static bool HasParentPath(this string path) =>
            path.Contains('/') && path.GetParentPath() != "";
        ...
        public static string ToLocalPath(this string globalPath) =>
            "Assets" + globalPath.Remove(Application.dataPath);
        ...
        public static string ClearDir(this string dir)
        {
            ...
            foreach (var r in diri.EnumerateFiles())
                r.Delete();
            foreach (var r in diri.EnumerateDirectories())
                r.Delete(true);
        ...
        public static List<string> ToPaths(this IEnumerable<string> guids) =>
            guids.Select(r => r.ToPath()).ToList();

        public static string GetFilename(this string path, bool withExtension = false) =>
            withExtension ? Path.GetFileName(path) : Path.GetFileNameWithoutExtension(path); // prev GetName
        ...
        public static string ToGuid(this string pathInProject) =>
            AssetDatabase.AssetPathToGUID(pathInProject);
        ...
        public static string GetGuid(this Object o) =>
            AssetDatabase.AssetPathToGUID(AssetDatabase.GetAssetPath(o));
        ...
        public static Object LoadGuid(this string guid) =>
            AssetDatabase.LoadAssetAtPath(guid.ToPath(), typeof(Object));

        public static T LoadGuid<T>(this string guid)
            where T : Object => AssetDatabase.LoadAssetAtPath<T>(guid.ToPath());

        public static List<string> FindAllAssetsOfType_guids(Type type) =>
            AssetDatabase.FindAssets("t:" + type.Name).ToList();

        public static List<string> FindAllAssetsOfType_guids(Type type, string path) =>
            AssetDatabase.FindAssets("t:" + type.Name, new[] { path }).ToList();

        public static List<T> FindAllAssetsOfType<T>()
            where T : Object =>
            FindAllAssetsOfType_guids(typeof(T)).Select(r => (T)r.LoadGuid()).ToList();

        public static List<T> FindAllAssetsOfType<T>(string path)
            where T : Object =>
            FindAllAssetsOfType_guids(typeof(T), path).Select(r => (T)r.LoadGuid()).ToList();

        public static T Reimport<T>(this T t)
            where T : Object
        {
        ...
        public static bool IsHidden(this string path) =>
            path.EndsWith("~") || File.Exists(path + "~");
        ...
        private static List<string> GetFilesRecursively(
            string path,
            System.Func<string, bool> criteria = null,
        ...
        public static bool ScriptHasDefineDisabled(Type scriptType) =>
            File.ReadLines(GetScriptPath(scriptType.Name)).First().StartsWith("#define DISABLED");
        ...
        public static void OpenFolder(string path)
        {
            ...
            m_ListAreaState
                .GetType()
                .GetField("m_SelectedInstanceIDs")
        ...
        public static EditorWindow OpenObjectPicker<T>(
            Object obj = null,
            ...
            where T : Object
        {
            ...
            return Resources
                    .FindObjectsOfTypeAll(
                        typeof(Editor).Assembly.GetType("UnityEditor.ObjectSelector")
                    )
                    .FirstOrDefault() as EditorWindow;
        }

        public static EditorWindow OpenColorPicker(
            System.Action<Color> colorChangedCallback,
            Color color,
            bool showAlpha = true,
            bool hdr = false
        )
        ...
        static MethodInfo removeEditorErrorsMethod = System
            .AppDomain.CurrentDomain.GetAssemblies()
            .FirstOrDefault(r => r.GetName().ToString().Contains("UnityEditor.CoreModule"))
            .GetTypes()
            .First(r => r.Name.Contains("LogEntry"))
    ...
    public static class VGUI
    {
        ...
        public static T Slider<T>(string name, T cur, T min, T max) =>
            Slider(ExpandWidthLabelRect(), name, cur, min, max);
        ...
        public static void _DrawResettableFieldCrossIcon(
            Rect rect,
            bool isObjectField = false,
            float brightness = .36f
        )
        {
            var iconRect = rect.SetWidthFromRight(20)
                .SetSizeFromMid(15)
                .MoveX(isObjectField ? -18 : 0)
                .MoveY(.5f);
        ...
        public static T ResettableField<T>(string name, T cur, T resetTo = default) =>
            ResettableField(ExpandWidthLabelRect(), name, cur, resetTo);

        public static void ResettableField<T>(
            Rect rect,
            string name,
            ref T cur,
            T resetTo = default
        ...
        public static T ResettableSlider<T>(
            Rect rect,
            string name,
            T cur,
            T min,
        ...
        public static T ResettableSlider<T>(
            ...
            T max,
            T resetTo = default
        ) => ResettableSlider(ExpandWidthLabelRect(), name, cur, min, max, resetTo);

        public static void ResettableSlider<T>(
            Rect rect,
            string name,
            ref T cur,
            T min,
        ...
        //     return cur;
        // }
        public static int ResettableFieldWithPlusMinsButtons(
            Rect rect,
            string name,
            int cur,
            int min,
        ...
        )
        {
            ...
            cur = ResettableField(
                rect.SetWidth(rect.width - buttonWidth * 2 - 4),
                name,
                cur,
                resetTo
            );
        ...
        public static int ResettableFieldWithPlusMinsButtons(
            ...
            ResettableFieldWithPlusMinsButtons(
        ...
        public static int ResettableFieldWithPlusMinsButtons(
            ...
            cur = ResettableFieldWithPlusMinsButtons(rect, name, cur, min, max, resetTo, increment);
        ...
        public static int ResettableFieldWithPlusMinsButtons(
            ...
            int resetTo = 0,
            int increment = 1
        ) => cur = ResettableFieldWithPlusMinsButtons(name, cur, min, max, resetTo, increment);

        public static int FieldWithPlusMinsButtons(
            Rect rect,
            string name,
            int cur,
            int min,
            int max,
            int increment = 1
        ) => ResettableFieldWithPlusMinsButtons(rect, name, cur, min, max, cur, increment);
        ...
        public static int FieldWithPlusMinsButtons(
            ...
            int max,
            int increment = 1
        ) => FieldWithPlusMinsButtons(ExpandWidthLabelRect(), name, cur, min, max, increment);
        ...
        public static int FieldWithPlusMinsButtons(
            ...
            int max,
            int increment = 1
        ) => cur = FieldWithPlusMinsButtons(rect, name, cur, min, max, increment);
        ...
        public static int FieldWithPlusMinsButtons(
            ...
            int max,
            int increment = 1
        ) => cur = FieldWithPlusMinsButtons(name, cur, min, max, increment);
        ...
        public static bool OnOffButton(string name, bool val) =>
            OnOffButton(ExpandWidthLabelRect(), name, val);
        ...
        public static bool Toggle(string name, bool val) =>
            Toggle(ExpandWidthLabelRect(), name, val);
        ...
        public static bool LeftToggle(string name, bool val, float offset = 0) =>
            LeftToggle(ExpandWidthLabelRect(), name, val, offset);
        ...
        public static Color SmallColorField(
            Rect rect,
            string name,
            Color val,
            bool showAlpha = true,
        ...
        public static Color SmallColorField(
            ...
            bool showAlpha = true,
            bool hdr = false
        ) => SmallColorField(ExpandWidthLabelRect().MoveX(-1), name, val, showAlpha, hdr);

        public static void SmallColorField(
            Rect rect,
            string name,
            ref Color val,
            bool showAlpha = true,
        ...
        public static void ObjectFieldWidhoutPicker<T>(Rect rect, string name, T val)
            where T : Object
        {
            var id = GUIUtility.GetControlID(
            ...
            void draw()
            {
                ...
                style.Draw(
                    fieldRect,
                    EditorGUIUtility.ObjectContent(val, typeof(T)),
                    id,
            ...
            void click()
            {
                if (!curEvent.isMouseDown || !rect.IsHovered() || curEvent.mouseButton != 0)
                    return;
        ...
        public static void ObjectFieldWidhoutPicker<T>(string name, T val)
            where T : Object => ObjectFieldWidhoutPicker(ExpandWidthLabelRect(), name, val);
        ...
        public static bool ClearButtonText(
            Rect rect,
            string text,
            bool val = false,
        ...
        )
        {
            ...
            var col = Color.white;
            if (drawPressed && val)
                col *= new Color(.35f, .57f, 1f, 1f) * 1.25f;
        ...
        public static void ColorPickerButton(
            Rect rect,
            Color color,
            System.Action<Color> colorChangedCallback,
            bool showAlphaBar = false,
            Color displayTint = default,
        ...
        )
        {
            ...
            rect.SetHeight(showAlphaBar ? rect.height - 2 : rect.height)
                .Draw(c * displayTint * (greyedOut ? GUIColors.greyedOutTint : Color.white));
            ...
            if (showAlphaBar)
            {
                ...
                rAlphaBar.Draw(
                    Greyscale(1, .65f) * (greyedOut ? GUIColors.greyedOutTint : Color.white)
                );
            }
        ...
        {
            var ints = Enumerable
                .Range((int)Mathf.Log(min, 2), (int)Mathf.Log(max, 2) - (int)Mathf.Log(min, 2) + 1)
                .Select(r => (int)Mathf.Pow(2, r))
                .ToArray();
            return EditorGUILayout.IntPopup(
                name,
                cur,
                ints.Select(r => r.ToString()).ToArray(),
                ints
            );
        }

        public static int HeightmapResolutionPopup(
            string name,
            int cur,
            int min = 513,
            int max = 16385
        )
        {
            var ints = Enumerable
                .Range(
                    (int)Mathf.Log(min - 1, 2),
                    (int)Mathf.Log(max - 1, 2) - (int)Mathf.Log(min - 1, 2) + 1
                )
                .Select(r => (int)Mathf.Pow(2, r))
                .Select(r => r + 1)
                .ToArray();
        ...
        public static T SelectorPopup<T>(
            Rect rect,
            string name,
            T selected,
            List<T> options,
        ...
        )
        {
            ...
            int r = 0;
            if (labelOffset < 0)
                r = EditorGUI.IntPopup(
                    rect,
                    name,
                    names.IndexOf(curName),
                    names.ToArray(),
                    ints.ToArray()
                );
            else
        ...
        public static void SelectorWithIcons<T>(
            ref T selected,
            List<T> options,
            System.Func<T, string> nameGetter,
            System.Func<T, Texture> textureGetter,
            System.Func<T, Rect?> uvsGetter,
            ref Vector2 scrollPos,
            ref bool renaming,
            ref string renamingTempName,
            float buttonSize = 60,
            float iconBorder = 1,
            bool showNoneOption = false,
            bool dragndropPossible = false,
            System.Action onAdd = null,
            System.Action<Object> onDragndrop = null,
            System.Action onRenameEnded = null,
            System.Action onSelectedClicked = null,
            System.Action<T> onRightclicked = null,
            Material iconMaterial = null,
            bool showPlusButton = true,
            string noneOptionText = "None",
            Texture2D noneOptionTexture = null
        )
        {
            ...
            void item(T t)
            {
                ...
                if (uvs != null)
                    GUI.DrawTextureWithTexCoords(
                        lastRect.Resize(iconBorder),
                        textureGetter(t),
                        uvs.GetValueOrDefault()
                    );
                ...
                if (t.Equals(selected_))
                    drawSelectedBlue(lastRect, nameGetter(t));
                ...
                if (t.Equals(selected_) && renaming_)
                {
                    ...
                    GUI.FocusControl("w");
                    newNamerenamingTempName_ = GUI.TextField(
                        nameRect(lastRect),
                        newNamerenamingTempName_
                    );
                }
                else
            ...
            void plus()
            {
                ...
                if (mouseIn)
                    EditorGUI.DrawRect(rect, Greyscale(.8f, .07f));

                if (!curEvent.isDragPerform || !mouseIn)
                    return;
            ...
            void none()
            {
                ...
                if (selected_ == null)
                    drawSelectedBlue(lastRect, noneOptionText);
        ...
        // prev Toggle
        public static bool ToggleLeftOld(
            string s,
            bool b,
            float offset = 6,
            float textOffset = -4,
        ...
        )
        {
            ...
            var rSlider = rect.MoveX(-valWindowSize - 4)
                .SetWidthFromRight(rect.width - EditorGUIUtility.labelWidth - valWindowSize - 6);
            var e = 2.71828;
            var k = (max - min) / (e - 1);
            var c0 = min - k;
        ...
        public static float ExpSlider(
            ...
            float max,
            bool showDecimal = true
        ) => ExpSlider(ExpandWidthLabelRect(), name, val, min, max, showDecimal);
        ...
        public static class GUIColors
        {
            ...
            public static Color pressedButtonBackground =>
                isDarkTheme
                    ? new Color(.48f, .76f, 1f, 1f) * 1.4f
                    : new Color(.48f, .7f, 1f, 1f) * 1.2f; // prev pressedButtonCol
            public static Color greyedOutTint => Greyscale(.7f);
            public static Color selectedBackground =>
                isDarkTheme ? new Color(.17f, .365f, .535f) : new Color(.2f, .375f, .555f) * 1.2f;
        }
        ...
        public static float GetLabelWidth(this string s) =>
            GUI.skin.label.CalcSize(new GUIContent(s)).x;
        ...
        public class WrappedEvent
        {
            ...
            public Vector2 mousePosition_screenSpace =>
            ...
            public bool holdingCmdOrCtrlOnly =>
                (e.modifiers == EventModifiers.Command || e.modifiers == EventModifiers.Control);
        ...
        public static WrappedEvent curEvent =>
            _curEvent ??= typeof(Event).GetFieldValue<Event>("s_Current").Wrap();
        ...
        public static Rect DrawWithRoundedCorners(this Rect rect, Color color, int cornerRadius)
        {
            ...
            cornerRadius = cornerRadius
                .Min((rect.height / 2).FloorToInt())
                .Min((rect.width / 2).FloorToInt());

            GUIStyle style;

            void getStyle()
            {
                if (_roundedStylesByCornerRadius.TryGetValue(cornerRadius, out style))
                    return;
                ...
                {
                    var sqrMagnitude = (
                        new Vector2(x - halfRes + .5f, y - halfRes + .5f)
                    ).sqrMagnitude;
        ...
        public static Rect DrawWithRoundedCorners(
            this Rect rect,
            Color color,
            float cornerRadius
        ) => rect.DrawWithRoundedCorners(color, cornerRadius.RoundToInt());

        static Dictionary<int, GUIStyle> _roundedStylesByCornerRadius =
        ...
        public static Rect DrawBlurred(this Rect rect, Color color, int blurRadius)
        {
            ...
            var croppedRectWidth = (rect.width * pixelsPerPoint)
                .RoundToInt()
                .Min(blurRadiusScaled * 2);
            var croppedRectHeight = (rect.height * pixelsPerPoint)
                .RoundToInt()
                .Min(blurRadiusScaled * 2);
            ...
            {
                if (
                    _blurredStylesByTextureSize.TryGetValue(
                ...
                for (int y = 0; y < textureHeight; y++)
                {
                    ...
                    for (
                        int xSample = (x - blurRadiusScaled).Max(blurRadiusScaled);
                        xSample <= (x + blurRadiusScaled).Min(textureWidth - 1 - blurRadiusScaled);
                        xSample++
                    )
                    for (
                        int ySample = (y - blurRadiusScaled).Max(blurRadiusScaled);
                        ySample <= (y + blurRadiusScaled).Min(textureHeight - 1 - blurRadiusScaled);
                        ySample++
                    )
                        sum += kernel[
                            blurRadiusScaled + xSample - x,
                            blurRadiusScaled + ySample - y
                        ];
            ...
            void draw()
            {
                ...
                style.Draw(
                    rect.SetSizeFromMid(rect.width + blurRadius * 2, rect.height + blurRadius * 2),
                    false,
                    false,
                    false,
                    false
        ...
        public static Rect DrawBlurred(this Rect rect, Color color, float blurRadius) =>
            rect.DrawBlurred(color, blurRadius.RoundToInt());

        static Dictionary<(int, int), GUIStyle> _blurredStylesByTextureSize =
            new Dictionary<(int, int), GUIStyle>();

        static void DrawCurtain(this Rect rect, Color color, int dir)
        {
            void genTextures()
            {
                if (_gradientTextures != null)
                    return;
                ...
                var pixels = Enumerable
                    .Range(0, 256)
                    .Select(r => Greyscale(1, (r / 255f).Smoothstep()));
        ...
        public static void Divider(float space = 15, float yOffset = 0)
        {
            ...
            lastRect
                .SetHeightFromMid(1)
                .SetWidthFromMid(lastRect.width - 16)
                .MoveY(yOffset)
                .Draw(isDarkTheme ? Color.white * .42f : Color.white * .72f);
        }
        ...
        public static class EditorIcons
        {
            ...
            {
                if (icons_byName.TryGetValue(iconNameOrPath, out var cachedResult))
                    return cachedResult;

                var icon =
                    typeof(EditorGUIUtility).InvokeMethod<Texture2D>("LoadIcon", iconNameOrPath)
                    as Texture2D;
        ...
        public static void MarkInteractive(this Rect rect)
        {
            ...
            var unclippedRect = (Rect)
                _mi_GUIClip_UnclipToWindow.Invoke(null, new object[] { rect });
        ...
        static PropertyInfo _pi_GUIView_current = typeof(Editor)
            .Assembly.GetType("UnityEditor.GUIView")
            .GetProperty("current", maxBindingFlags);
        static MethodInfo _mi_GUIView_MarkHotRegion = typeof(Editor)
            .Assembly.GetType("UnityEditor.GUIView")
            .GetMethod("MarkHotRegion", maxBindingFlags);
        static MethodInfo _mi_GUIClip_UnclipToWindow = typeof(GUI)
            .Assembly.GetType("UnityEngine.GUIClip")
            .GetMethod("UnclipToWindow", maxBindingFlags, null, new[] { typeof(Rect) }, null);
