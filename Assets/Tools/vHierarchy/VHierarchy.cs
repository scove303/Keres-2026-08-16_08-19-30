...
namespace VHierarchy
{
    public static class VHierarchy
    {
        static void GameObjectRowGUI(GameObject go, Rect rowRect)
        {
            ...
            void setState()
            {
                void set_isRowSelected()
                {
                    ...
                    var dragSelectionList = treeViewController
                        ?.GetFieldValue("m_DragSelection")
                        ?.GetFieldValue<List<int>>("m_List");
#else
                    var dragSelectionList = treeViewController?.GetFieldValue<List<int>>(
                        "m_DragSelection"
                    );
#endif
                    ...
                    isRowSelected = dragging
                        ? (dragSelectionList.Contains(go.GetInstanceID()))
                        : Selection.Contains(go);
                }
                void set_isRowBeingRenamed()
                {
                    ...
                    isRowBeingRenamed =
                        ...
                        && isRowSelected
                        && treeViewController
                            ?.GetMemberValue("state")
                            ?.GetMemberValue("renameOverlay")
                            ?.InvokeMethod<bool>("IsRenaming") == true;
                }
                void set_isTreeFocused()
                {
                    ...
                    isTreeFocused =
                        ...
                        && GUIUtility.keyboardControl
                            == hierarchyWindow
                                ?.GetMemberValue("sceneHierarchy")
                                ?.GetMemberValue<int>("m_TreeViewKeyboardControlID");
                }
            ...
            void drawing()
            {
                ...
                var showBackgroundColor =
                    goData != null && goData.colorIndex.IsInRange(1, VHierarchyPalette.colorsCount); // && !(isRowSelected && isHierarchyFocused);
                ...
                var showDefaultIcon =
                    ...
                    && (
                        isRowBeingRenamed
                        || (
                            !VHierarchyMenu.minimalModeEnabled
                            || (
                ...
                void hideName()
                {
                    if (!showBackgroundColor && (showCustomIcon || showDefaultIcon))
                        return;
                ...
                void backgroundColor()
                {
                    ...
                    if (!isRowSelected)
                        colorRect = colorRect.AddHeightFromMid(
                            EditorGUIUtility.pixelsPerPoint >= 2 ? -.5f : -1
                        );
                    ...
                    if (PrefabUtility.HasPrefabInstanceAnyOverrides(go, false) && !hasLeftGradient)
                        colorRect = colorRect.AddWidthFromRight(
                            EditorGUIUtility.pixelsPerPoint >= 2 ? -2.5f : -3
                        );

                    var leftGradientWith = go.transform.parent ? 22 : 0;
                    var rightGradientWidth = (fullRowRect.width * .77f).Min(
                        colorRect.width - leftGradientWith
                    );
                    ...
                    var flatColorRect = colorRect
                        .SetX(leftGradientRect.xMax)
                        .SetXMax(rightGradientRect.x);

                    var colorWithFlatness = palette
                        ? palette.colors[goData.colorIndex - 1]
                        : VHierarchyPalette.GetDefaultColor(goData.colorIndex - 1);
                ...
                void triangle()
                {
                    ...
                    GUI.DrawTexture(
                        triangleRect,
                        EditorIcons.GetIcon(IsExpanded(go) ? "IN_foldout_on" : "IN_foldout")
                    );

                    if (!makeTriangleBrighter)
                        return;
                ...
                void name()
                {
                    ...
                    if (makeNameBrighter)
                        SetGUIColor(
                            Greyscale(
                                !go.activeInHierarchy ? 1.4f
                                : isRowSelected ? 1
                                : .9f
                            )
                        );

                    GUI.skin.GetStyle(styleName)
                        .Draw(
                            nameRect,
                            go.name,
                            false,
                ...
                void customIcon()
                {
                    ...
                    var iconRect = rowRect.SetWidth(16);
                    var iconNameOrPath =
                        goData.iconNameOrGuid.Length == 32
                            ? goData.iconNameOrGuid.ToPath()
                            : goData.iconNameOrGuid;

                    SetGUIColor(go.activeInHierarchy ? Color.white : Greyscale(1, .4f));

                    GUI.DrawTexture(
                        iconRect,
                        EditorIcons.GetIcon(iconNameOrPath) ?? Texture2D.blackTexture
                    );
                ...
                void hierarchyLines()
                {
                    if (!VHierarchyMenu.hierarchyLinesEnabled)
                        return;
                    ...
                    void drawVerticals()
                    {
                        for (int i = 0; i < depth; i++)
                            if (!hierarchyLines_verticalGaps.Contains(i))
                                rowRect
                                    .SetX(53 + i * 14 - lineThickness / 2)
                                    .SetWidth(lineThickness)
                                    .SetHeight(
                                        isLastChild(go.transform) && i == depth - 1
                                            ? 8 + lineThickness / 2
                                            : 16
                                    )
                ...
                void zebraStriping()
                {
                    if (!VHierarchyMenu.zebraStripingEnabled)
                        return;
                    if (isRowSelected)
                        return;
                    if (goData?.colorIndex == 1)
                        return;
            ...
            {
                if (!VHierarchyMenu.componentMinimapEnabled)
                    return;
                ...
                void componentButton(Rect buttonRect, Component component)
                {
                    void componentIcon()
                    {
                        ...
                        var isActive =
                            (buttonRect.IsHovered() && curEvent.holdingAlt)
                            || VHierarchyComponentWindow.floatingInstance?.component == component;
                        ...
                        SetGUIColor(
                            Greyscale(
                                1,
                                isActive
                                    ? (isPressed ? pressedOpacity : activeOpacity)
                                    : normalOpacity
                            )
                        );
                    ...
                    void mouseUp()
                    {
                        ...
                        var position = EditorGUIUtility.GUIToScreenPoint(
                            new Vector2(rowRect.xMax + 25, rowRect.y)
                        );
                ...
                void transformComponent()
                {
                    ...
                    componentButton(
                        fullRowRect.SetWidth(13).MoveX(1.5f),
            ...
            {
                if (!VHierarchyMenu.activationToggleEnabled)
                    return;
                if (!isRowHovered)
                    return;
                if (curEvent.holdingAlt)
                ...
                if (newActiveSelf == go.activeSelf)
                    return;
            ...
            void altDrag()
            {
                ...
                void mouseDrag()
                {
                    if (!curEvent.isMouseDrag)
                        return;
                    if ((curEvent.mousePosition - mouseDownPos).magnitude < 5)
                        return;
                    if (!rowRect.Contains(mouseDownPos))
                        return;
                    if (!rowRect.Contains(curEvent.mousePosition - curEvent.mouseDelta))
                        return;
            ...
            void altClick()
            {
                ...
                void mouseUp()
                {
                    ...
                    var editMultiSelection =
                        Selection.gameObjects.Length > 1 && Selection.gameObjects.Contains(go);

                    var gosToEdit = (
                        editMultiSelection ? Selection.gameObjects : new[] { go }
                    ).ToList();

                    if (
                        VHierarchyPaletteWindow.instance
                        && VHierarchyPaletteWindow.instance.gameObjects.SequenceEqual(gosToEdit)
                    )
                    {
                    ...
                    var position = EditorGUIUtility.GUIToScreenPoint(
                        new Vector2(openNearRect.x - 14, openNearRect.y + 18)
                    );
        ...
        static void SceneRowGUI(Scene scene, Rect rowRect)
        {
            void collapseAll()
            {
                if (!VHierarchyMenu.collapseAllButtonEnabled)
                    return;

                var buttonRect = rowRect
                    .SetWidthFromRight(18)
                    .MoveX(VHierarchyMenu.editLightingButtonEnabled ? -22 : -4);
                ...
                GUI.Label(
                    buttonRect.Resize(1.5f).MoveY(-.5f),
                    EditorGUIUtility.IconContent("PreviewCollapse")
                );
                ...
                foreach (var iid in expandedIds)
                    if (
                        EditorUtility.InstanceIDToObject(iid) is GameObject expandedGo
                        && expandedGo.scene == scene
                    )
                ...
                expandQueue_toAnimate = expandedRoots
                    .Select(r => new ExpandQueueEntry
                    {
                        instanceId = r.GetInstanceID(),
                        expand = false,
                    })
                    .OrderBy(r => VisibleRowIndex(r.instanceId))
                    .ToList();
            ...
            void lighting()
            {
                if (!VHierarchyMenu.editLightingButtonEnabled)
                    return;
                ...
                VHierarchyLightingWindow.CreateInstance(
                    EditorGUIUtility.GUIToScreenPoint(curEvent.mousePosition) + new Vector2(8, -8)
                );
        ...
        static void RowGUI(int instanceId, Rect rowRect)
        {
            if (EditorWindow.focusedWindow is EditorWindow focusedWindow)
                if (
                    focusedWindow.GetType() == t_SceneHierarchyWindow
                    && focusedWindow != hierarchyWindow
                )
        ...
        static void CheckShortcuts() // globalEventHandler
        {
            ...
            {
                if (hierarchyWindow == EditorWindow.mouseOverWindow)
                    return;
            ...
            void toggleExpanded()
            {
                if (!hoveredGo)
                    return;
                if (curEvent.holdingAnyModifierKey)
                    return;
                if (!curEvent.isKeyDown || curEvent.keyCode != KeyCode.E)
                    return;
                if (Tools.viewTool == ViewTool.FPS)
                    return;
                if (!VHierarchyMenu.toggleExpandedEnabled)
                    return;
                ...
                if (hoveredGo.transform.childCount == 0)
                    return;

                SetExpandedWithAnimation(
                    hoveredGo.GetInstanceID(),
                    !expandedIds.Contains(hoveredGo.GetInstanceID())
                );
            ...
            void toggleActive()
            {
                ...
                if (curEvent.holdingAnyModifierKey)
                    return;
                if (!curEvent.isKeyDown || curEvent.keyCode != KeyCode.A)
                    return;
                if (Tools.viewTool == ViewTool.FPS)
                    return;
                if (!VHierarchyMenu.toggleActiveEnabled)
                    return;
            ...
            void delete()
            {
                ...
                if (curEvent.holdingAnyModifierKey)
                    return;
                if (!curEvent.isKeyDown || curEvent.keyCode != KeyCode.X)
                    return;
                if (!VHierarchyMenu.deleteEnabled)
                    return;
            ...
            {
                if (
                    curEvent.modifiers != (EventModifiers.Shift | EventModifiers.Command)
                    && curEvent.modifiers != (EventModifiers.Shift | EventModifiers.Control)
                )
                    return;
                if (!curEvent.isKeyDown || curEvent.keyCode != KeyCode.E)
                    return;
                if (!VHierarchyMenu.collapseEverythingEnabled)
                    return;
            ...
            void collapseEverythingElse()
            {
                ...
                if (!curEvent.isKeyDown || curEvent.keyCode != KeyCode.E)
                    return;
                if (!VHierarchyMenu.collapseEverythingElseEnabled)
                    return;
                ...
                foreach (var iid in expandedIds.ToList())
                    if (
                        EditorUtility.InstanceIDToObject(iid) is GameObject expandedGo
                        && !parents.Contains(expandedGo)
                        && expandedGo != hoveredGo
                    )
                        toCollapse.Add(expandedGo);

                expandQueue_toAnimate = toCollapse
                    .Select(r => new ExpandQueueEntry
                    {
                        instanceId = r.GetInstanceID(),
                        expand = false,
                    ...
                    .Append(
                        new ExpandQueueEntry
                        {
                            instanceId = hoveredGo.GetInstanceID(),
                            expand = true,
                        }
                    )
                    .OrderBy(r => VisibleRowIndex(r.instanceId))
            ...
            void focus()
            {
                if (!curEvent.isKeyDown)
                    return;
                if (curEvent.modifiers != EventModifiers.None)
                    return;
                if (curEvent.keyCode != KeyCode.F)
                    return;
                if (SceneView.sceneViews.Count == 0)
                    return;
                if (!hoveredGo)
                    return;
                if (!VHierarchyMenu.focusEnabled)
                    return;
                ...
                if (!sv || !sv.hasFocus)
                    sv =
                        SceneView
                            .sceneViews.ToArray()
                            .FirstOrDefault(r => (r as SceneView).hasFocus) as SceneView;

                if (!sv)
                    (
                        sv = SceneView.lastActiveSceneView ?? SceneView.sceneViews[0] as SceneView
                    ).Focus();
        ...
        {
            if (treeViewController.GetPropertyValue<bool>("animatingExpansion"))
                return;

            if (!expandQueue_toAnimate.Any())
            {
                if (!expandQueue_toCollapseAfterAnimation.Any())
                    return;
        ...
        static bool IsVisible(GameObject go) =>
            !go.transform.parent
            || (
                IsExpanded(go.transform.parent.gameObject)
                && IsVisible(go.transform.parent.gameObject)
            );
        ...
        static void SetExpanded(int instanceId, bool expanded) =>
            treeViewController
                .GetPropertyValue("data")
                .InvokeMethod("SetExpanded", instanceId, expanded); // static void SetExpanded(int instanceId, bool expanded) => hierarchyWindow.InvokeMethod("SetExpanded", instanceId, expanded);

        static int VisibleRowIndex(int instanceId) =>
            treeViewController.GetPropertyValue("data").InvokeMethod<int>("GetRow", instanceId);

        public static string GetComponentName(Component component)
        {
            var s = new GUIContent(
        ...
        public static Texture GetComponentIcon(Component component)
        {
            ...
            if (!componentIcons_byType.ContainsKey(component.GetType()))
                componentIcons_byType[component.GetType()] = EditorGUIUtility
                    .ObjectContent(component, component.GetType())
                    .image;

            return componentIcons_byType[component.GetType()];
        }

        static Dictionary<System.Type, Texture> componentIcons_byType =
            new Dictionary<Type, Texture>();

        public static GameObjectData GetGameObjectData(GameObject go, bool createDataIfDoesntExist)
        {
            if (!data)
                return null;
            if (firstDataCacheLayer.TryGetValue(go, out var cachedResult))
                return cachedResult;
            ...
            void sceneObject()
            {
                ...
                var originalSceneGuid =
                    cache.originalSceneGuids_byInstanceId.GetValueOrDefault(go.GetInstanceID())
                    ?? currentSceneGuid;

                void getSceneDataFromComponents()
                {
                    if (!dataComponents_byScene.ContainsKey(go.scene))
                        dataComponents_byScene[go.scene] = Resources
                            .FindObjectsOfTypeAll<VHierarchyDataComponent>()
                            .FirstOrDefault(r => r.gameObject?.scene == go.scene);
                ...
                void createSceneIdMap()
                {
                    ...
                    if (sceneData == null)
                        return;
                    if (currentSceneGuid != originalSceneGuid)
                        return;
                ...
                void updateSceneIdMapAndOriginalSceneGuids()
                {
                    ...
                    var curInstanceIdsHash =
                        go.scene.GetRootGameObjects().FirstOrDefault()?.GetInstanceID() ?? 0;
                    var curGlobalIdsHash = sceneData.goDatas_byGlobalId.Keys.Aggregate(
                        0,
                        (hash, r) => hash ^= r.GetHashCode()
                    );
                    ...
                    var instanceIds = globalIds
                        .Select(r => Application.isPlaying ? r.UnpackForPrefab() : r)
                    ...
                    void fillIdMap()
                    {
                        sceneIdMap.globalIds_byInstanceId =
                            new SerializableDictionary<int, GlobalID>();
                    ...
                    {
                        for (int i = 0; i < instanceIds.Length; i++)
                            cache.originalSceneGuids_byInstanceId[instanceIds[i]] =
                                currentSceneGuid;
                    }
                ...
                void getGoData()
                {
                    ...
                    if (sceneIdMap == null)
                        return;
                    if (
                        !sceneIdMap.globalIds_byInstanceId.TryGetValue(
                            go.GetInstanceID(),
                            out var globalId
                        )
                    )
                ...
                void moveGoDataToCurrentSceneGuid() // totest
                {
                    ...
                    var currentSceneData =
                        dataComponents_byScene.GetValueOrDefault(go.scene)?.sceneData
                        ?? data.sceneDatas_byGuid.GetValueOrDefault(currentSceneGuid);
                    ...
                    originalSceneData.goDatas_byGlobalId.Remove(
                        originalSceneData.goDatas_byGlobalId.First(r => r.Value == goData).Key
                    );
            ...
            void prefabObject()
            {
                if (!(StageUtility.GetCurrentStage() is PrefabStage prefabStage))
                    return;
                ...
                void calcGlobalId()
                {
                    ...
                    if (rawFileId == 0) // happens for prefab variants in unity 6
                        rawFileId = (long)
                            t_Unsupported.InvokeMethod<ulong>("GetOrGenerateFileIDHint", go);
                    ...
                    var fileId =
                        ((long)rawFileId ^ (long)rawGlobalId.globalObjectId.targetPrefabId)
                        & 0x7fffffffffffffff;
            ...
            void prefabInstance_editMode()
            {
                ...
                void tryGetForSourceGo(GameObject sourceGo)
                {
                    ...
                    cache.prefabInstanceGlobalIds_byInstanceIds[go.GetInstanceID()] =
                        sourceGoGlobalId;
                    ...
                    if (goData == null)
                        ...
                        {
                            if (
                                PrefabUtility.GetCorrespondingObjectFromSource(sourceGo)
                                is GameObject previousSourceGo
                            )
            ...
            void prefabInstance_playmode()
            {
                ...
                if (
                    !cache.prefabInstanceGlobalIds_byInstanceIds.TryGetValue(
                        go.GetInstanceID(),
                        out var globalId
                    )
                )
        ...
        public static Dictionary<GameObject, GameObjectData> firstDataCacheLayer =
            new Dictionary<GameObject, GameObjectData>(); // cleared on data serialization callbacks, ie when data is added or removed

        public static Dictionary<Scene, VHierarchyDataComponent> dataComponents_byScene =
            new Dictionary<Scene, VHierarchyDataComponent>();
        ...
        static void DuplicateSceneData(string originalSceneGuid, string duplicatedSceneGuid)
        {
            ...
            {
                var duplicatedGlobalId = new GlobalID(
                    kvp.Key.ToString().Replace(originalSceneGuid, duplicatedSceneGuid)
                );
                var duplicatedGoData = new GameObjectData()
                {
                    colorIndex = kvp.Value.colorIndex,
                    iconNameOrGuid = kvp.Value.iconNameOrGuid,
                };
        ...
        {
            if (curEvent.commandName != "Duplicate" && curEvent.commandName != "Paste")
                return;

            var copiedAssets_paths = new List<string>();

            var assetClipboard = typeof(Editor)
                .Assembly.GetType("UnityEditor.AssetClipboardUtility")
                .GetMemberValue("assetClipboard")
                .InvokeMethod<IEnumerator>("GetEnumerator");

            while (assetClipboard.MoveNext())
                copiedAssets_paths.Add(
                    assetClipboard.Current.GetMemberValue<GUID>("guid").ToString().ToPath()
                );

            var originalScenePath = copiedAssets_paths.FirstOrDefault(r =>
                File.Exists(r) && new FileInfo(r).Length == new FileInfo(importedScenePath).Length
            );
            var originalSceneGuid = originalScenePath.ToGuid();
            var duplicatedSceneGuid = importedScenePath.ToGuid();

            if (!data.sceneDatas_byGuid.ContainsKey(originalSceneGuid))
                return;
            if (data.sceneDatas_byGuid.ContainsKey(duplicatedSceneGuid))
                return;
        ...
        static void Init()
        {
            if (VHierarchyMenu.pluginDisabled)
                return;
            ...
            void subscribe()
            {
                ...
                EditorApplication.hierarchyWindowItemOnGUI =
                    RowGUI + EditorApplication.hierarchyWindowItemOnGUI;
                ...
                var globalEventHandler =
                    typeof(EditorApplication).GetFieldValue<EditorApplication.CallbackFunction>(
                        "globalEventHandler"
                    );
                typeof(EditorApplication).SetFieldValue(
                    "globalEventHandler",
                    CheckShortcuts + (globalEventHandler - CheckShortcuts)
                );

                var projectWasLoaded =
                    typeof(EditorApplication).GetFieldValue<UnityEngine.Events.UnityAction>(
                        "projectWasLoaded"
                    );
                typeof(EditorApplication).SetFieldValue(
                    "projectWasLoaded",
                    (projectWasLoaded - ClearCacheOnProjectLoaded) + ClearCacheOnProjectLoaded
                );
            }
            void loadData()
            {
                data = AssetDatabase.LoadAssetAtPath<VHierarchyData>(
                    EditorPrefs.GetString("vHierarchy-lastKnownDataPath-" + GetProjectId())
                );
                ...
                data = AssetDatabase
                    .FindAssets("t:VHierarchyData")
                    .Select(guid => AssetDatabase.LoadAssetAtPath<VHierarchyData>(guid.ToPath()))
                    .FirstOrDefault();
                ...
                EditorPrefs.SetString(
                    "vHierarchy-lastKnownDataPath-" + GetProjectId(),
                    data.GetPath()
                );
            }
            void loadPalette()
            {
                palette = AssetDatabase.LoadAssetAtPath<VHierarchyPalette>(
                    EditorPrefs.GetString("vHierarchy-lastKnownPalettePath-" + GetProjectId())
                );

                if (palette)
                    return;

                palette = AssetDatabase
                    .FindAssets("t:VHierarchyPalette")
                    .Select(guid => AssetDatabase.LoadAssetAtPath<VHierarchyPalette>(guid.ToPath()))
                    .FirstOrDefault();
                ...
                EditorPrefs.SetString(
                    "vHierarchy-lastKnownPalettePath-" + GetProjectId(),
                    palette.GetPath()
                );
            }
            ...
            void migrateDataFromV1()
            {
                ...
                if (
                    EditorPrefs.GetBool(
                        "vHierarchy-dataMigrationFromV1Attempted-" + GetProjectId(),
                        false
                    )
                )
                    return;
                ...
                if (lines.Length < 15 || !lines[14].Contains("sceneDatasByGuid"))
                    return;
                ...
                void parseGlobalIdLists_andCountGoDatasByInstanceId()
                {
                    ...
                    for (int i = 0; i < lines.Length; i++)
                    {
                        ...
                        void startParsing()
                        {
                            if (!line.Contains("goDatasByGlobalId"))
                                return;
                        ...
                        void parse()
                        {
                            if (!parsingGlobalIdList)
                                return;
                            if (!line.Contains("- GlobalObjectId"))
                                return;

                            var startIndex = line.IndexOf("- ") + 2;

                            if (startIndex < line.Length)
                                globalIdLists[parsingGlobalIdListAtIndex]
                                    .Add(line.Substring(startIndex));
                            else
                                globalIdLists[parsingGlobalIdListAtIndex].Add("");
                        }
                        void stopParsing_andCountDatasByInstanceId()
                        {
                            if (!line.Contains("goDatasByInstanceId"))
                                return;

                            parsingGlobalIdList = false;

                            var goDatasByInstanceId_keysLine = lines[i + 1];
                            var goDatasByInstanceId_count =
                                (goDatasByInstanceId_keysLine.Length - 14) / 8;
        ...
        static EditorWindow hierarchyWindow
        {
            get
            {
                if (
                    ...
                    && _hierarchyWindow.GetType() != t_SceneHierarchyWindow
                ) // happens on 2022.3.22f1 with enter playmode options on
                    _hierarchyWindow = null;

                if (_hierarchyWindow == null)
                    _hierarchyWindow =
                        Resources.FindObjectsOfTypeAll(t_SceneHierarchyWindow).FirstOrDefault()
                        as EditorWindow;
        ...
        static object treeViewController =>
            hierarchyWindow?.GetFieldValue("m_SceneHierarchy").GetFieldValue("m_TreeView"); // recreated on prefab mode enter/exit

        static Type t_SceneHierarchyWindow = typeof(Editor).Assembly.GetType(
