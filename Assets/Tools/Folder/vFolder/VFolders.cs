...
namespace VFolders
{
    public static class VFolders
    {
        static void RowGUI(string guid, Rect rowRect)
        {
            ...
            void setObjects()
            {
                ...
                void set_treeItem_byRect()
                {
                    ...
                    treeItem = tree.GetItem(rowIndex);
                ...
                void set_treeItem_byInstanceId()
                {
                    ...
                    try { instanceId = typeof(AssetDatabase).InvokeMethod("GetMainAssetOrInProgressProxyInstanceID", guid.ToPath()); }
                    catch
                    {
                        var asset = AssetDatabase.LoadMainAssetAtPath(guid.ToPath());
                        if (asset) instanceId = asset.GetInstanceID();
                    }

                    if (instanceId == null) return;

                    treeItem = tree.FindItem(instanceId);
        ...
        class Tree
        {
            ...
            public bool animatingExpansion => treeViewController != null && treeViewController.GetPropertyValue<bool>("animatingExpansion", exceptionIfNotFound: false);

            public bool IsExpanded(TreeViewItem treeItem) => expandedIds.Contains(treeItem.id);

            public TreeViewItem GetItem(int rowIndex)
            {
                if (data == null) return null;
                if (rowIndex < 0) return null;

                var item = InvokeTreeViewItem(data, "GetItem", rowIndex);
                if (item != null) return item;

                var rows = GetRows(data);

                if (rows is IList list)
                    return rowIndex < list.Count ? list[rowIndex] as TreeViewItem : null;
                ...
                if (rows is IEnumerable enumerable)
                {
                    ...
                    {
                        if (i == rowIndex) return row as TreeViewItem;
            ...
            public TreeViewItem FindItem(object instanceId)
            {
                if (treeViewController == null) return null;
                if (instanceId == null) return null;

                return InvokeTreeViewItem(treeViewController, "FindItem", instanceId);
            ...
            public static MethodInfo mi_data_GetItem = typeof(Editor).Assembly.GetType("UnityEditor.IMGUI.Controls.ITreeViewDataSource")?.GetMethod("GetItem", maxBindingFlags, null, new[] { typeof(int) }, null);

            static TreeViewItem InvokeTreeViewItem(object target, string methodName, params object[] args)
            {
                if (target == null) return null;

                var methodInfos = new List<MethodInfo>();

                if (methodName == "GetItem" && args.Length == 1 && args[0] is int && mi_data_GetItem != null)
                    methodInfos.Add(mi_data_GetItem);

                var concreteMethodInfo = GetMethod(target.GetType(), methodName, args.Select(r => r.GetType()).ToArray());

                if (concreteMethodInfo != null && !methodInfos.Contains(concreteMethodInfo))
                    methodInfos.Add(concreteMethodInfo);

                foreach (var methodInfo in methodInfos)
                    try
                    {
                        if (methodInfo.Invoke(target, args) is TreeViewItem treeViewItem)
                            return treeViewItem;
                    }
                    catch { }
            ...
            static object GetRows(object data)
            {
                if (data == null) return null;

                var methodInfo = GetMethod(data.GetType(), "GetRows");

                if (methodInfo != null)
                    try { return methodInfo.Invoke(data, null); }
                    catch { }

                return data.GetMemberValue("rows", exceptionIfNotFound: false) ??
                       data.GetMemberValue("m_Rows", exceptionIfNotFound: false);

            }

            static MethodInfo GetMethod(Type type, string methodName, params Type[] argumentTypes)
            {
                if (type == null) return null;

                MethodInfo getMethod(Type curType)
                {
                    foreach (var methodInfo in curType.GetMethods(maxBindingFlags))
                    {
                        ...
                        if (parameters.Length != argumentTypes.Length) continue;

                        if (parameters.Select(r => r.ParameterType).SequenceEqual(argumentTypes))
                            return methodInfo;
                    ...
                    foreach (var methodInfo in curType.GetMethods(maxBindingFlags))
                    {
                        ...
                        for (var i = 0; i < parameters.Length; i++)
                            compatible &= parameters[i].ParameterType.IsAssignableFrom(argumentTypes[i]);

                        if (compatible) return methodInfo;
                ...
                for (var curType = type; curType != null; curType = curType.BaseType)
                    if (getMethod(curType) is MethodInfo methodInfo)
                        return methodInfo;

                foreach (var interfaceType in type.GetInterfaces())
                    if (getMethod(interfaceType) is MethodInfo methodInfo)
                        return methodInfo;
